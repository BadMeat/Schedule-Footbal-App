 ...
 */
class NextMatchFragment : Fragment(), AnkoComponent<Context>, MatchView, SearchView.OnQueryTextListener {

    private lateinit var progressBar: ProgressBar

    private var nextEvent: MutableList<MatchEvent> = mutableListOf()
    ...
    override fun showAllLeague(data: List<League>) {
        println("Sudah Masuk sinis")
        if (data.isEmpty()) {
            println("Datanya Kosong cook")
        } else {
            println("Data Berisis")
        }
        ...
        val spinnerAdapter = ArrayAdapter(ctx, android.R.layout.simple_spinner_dropdown_item, myLeague)
        spinner.adapter = spinnerAdapter

        spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View, position: Int, id: Long) {
                passKey = myListKey[position]
                passPreseneter.getNextEvent(passKey)
            }
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        if (isNetworkAvailable(context)) {
            setHasOptionsMenu(true)
            passAdapter = PassAdapter(nextEvent) {
                startActivity(
